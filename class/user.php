	...
	function bulk_action($args){
		...
		if(count($users)){
		 	foreach($users as $user){
				$result = '';
				$user_obj = $this->mng_core->mng_get_user_info( $user, 'id' );
				if ($user_obj != false){
				 	switch($user_edit_action) {
						...
						case 'change-role':
					 		if ($new_role){
								if($user != $username){
									...
									$result = array('error' => 'Cannot change role to user assigned for wpmanagerpro.');
								}
							} else {
						...
						case 'delete-user':
							if ($user != $username) {
								if(!$this->last_admin($user_obj)){
									if($reassign_user){
										$to_user = $this->mng_core->mng_get_user_info( $reassign_user );
										if ($to_user != false){
								...
								$result = array('error' => 'Cannot delete user assigned for wpmanagerpro.');
							}
							break;
						default:
