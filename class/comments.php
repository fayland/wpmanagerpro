...
class MNG_Comment {
    ...
    function get_comments($args) {
		...
		foreach ($args as $checkbox => $checkbox_val) {
			if ($checkbox_val === "on") {
				$status_val = str_replace("get_comments_","",$checkbox);
	...
	function change_status($args) {
    	...
    	$status = $args['status'];
       	...
       	if ( 'approve' === $status )
			$status_sql = '1';
		elseif ( 'unapprove' === $status || $status === 'untrash' || $status === 'unspam' )
			$status_sql = '0';
		elseif ( 'spam' === $status )
			$status_sql = 'spam';
		elseif ( 'trash' === $status )
		...
		if (!empty($comment_id)) {
			...
			if ($status === 'delete') {
				...
				$success = $wpdb->query($update_query);
				$wpdb->query($delete_query);
			} else {
				$update_query = "UPDATE $wpdb->comments SET comment_approved = '" . $status_sql . "' where comment_ID = ".$comment_id;
				$success = $wpdb->query($update_query);
			}
			return 1;
		}else{
	...
	function bulk_action($args) {
		...
		return array('success' => "comments updated");
	}
	...
	function reply_comment($args) {
		...
		if(!empty($comment_id) && !empty($reply_text)){
			$comment_id = (int) $comment_id;
			$post_id = $wpdb->get_var( "SELECT comment_post_ID FROM $wpdb->comments WHERE comment_ID = " . $comment_id );
			...
			if (! $lastid) {
				return array("error" => "Error when insert comment into db.");
			}
